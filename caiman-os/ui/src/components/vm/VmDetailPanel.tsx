...
import { X, Terminal, RefreshCw, StopCircle, Play, Cpu, MemoryStick, Network, HardDrive } from 'lucide-react'
      ...
      {detailOpen && vm && (
        ...
        >
          ...
          <div className="p-2.5 border-t border-caiman-border flex flex-wrap gap-1.5">
            <button className="btn-primary flex-1 text-[9px]">
            ...
            {vm.status === 'RUNNING' && (
              <button className="btn btn-danger flex-1 text-[9px]">
                <StopCircle size={10} /> Stop
              </button>
            )}
            {vm.status === 'STOPPED' && (
              <button className="btn flex-1 text-[9px] border-caiman-border2 text-caiman-bright">
                <Play size={10} /> Start
              </button>
            )}
          </div>
